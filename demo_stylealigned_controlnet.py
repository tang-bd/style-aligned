...
def style_aligned_controlnet(
    ref_style_prompt, depth_map, ref_image, img_generation_prompt, seed
):
    try:
        if depth_map == True:
            image = load_image(ref_image)
            depth_image = pipeline_calls.get_depth_map(
                image, feature_processor, depth_estimator
            )
        else:
        ...
        latents = torch.randn(1 + num_images_per_prompt, 4, 128, 128, generator=gen).to(
            pipeline.unet.dtype
        )

        images = pipeline_calls.controlnet_call(
            pipeline,
        ...
        return [images[0], depth_image] + images[1:], gr.Image(
            value=images[0], visible=True
        )
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(variant="panel"):
            ...
            ref_style_prompt = gr.Textbox(
                ...
                placeholder="a poster in <style name> style",
            )
            with gr.Row(variant="panel"):
                # Checkbox for using controller depth-map
                depth_map = gr.Checkbox(
                    label="Depth-map",
                )
                seed = gr.Number(
                    value=1234,
            ...
            ref_style_image = gr.Image(
                visible=False, label="Reference style image", scale=1
            )

        with gr.Column(variant="panel"):
            # Image upload option for uploading a reference image for controlnet
            ref_image = gr.Image(label="Upload the reference image", type="filepath")
    ...
    gallery = gr.Gallery(
        label="Style-Aligned ControlNet - Generated images",
        elem_id="gallery",
        columns=5,
        rows=1,
        object_fit="contain",
    ...
    gr.Examples(
        examples=[
            [
                "A couple sitting a wooden bench, in colorful clay animation, claymation style.",
                True,
                "example_image/train.png",
                "A train in colorful clay animation, claymation style.",
            ],
            [
                "A couple sitting a wooden bench, in colorful clay animation, claymation style.",
                False,
                "example_image/sun.png",
                "Sun in colorful clay animation, claymation style.",
            ],
            [
                "A poster in a papercut art style.",
                False,
                "example_image/A.png",
                "Letter A in a papercut art style.",
                None,
            ],
            [
                "A bull in a low-poly, colorful origami style.",
                True,
                "example_image/whale.png",
                "A whale in a low-poly, colorful origami style.",
                None,
            ],
            [
                "An image in ancient egyptian art style, hieroglyphics style.",
                True,
