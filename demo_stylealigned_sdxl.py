...
def style_aligned_sdxl(
    ...
    try:
        ...
        sets_of_prompts = [
            prompt + " in the style of " + style_prompt
            for prompt in [
                initial_prompt1,
                initial_prompt2,
                initial_prompt3,
...
with gr.Blocks() as demo:
    ...
    with gr.Group():
        with gr.Column():
            with gr.Accordion(
                label="Enter upto 5 different initial prompts", open=True
            ):
                with gr.Row(variant="panel"):
                    # Textboxes for initial prompts
                    initial_prompt1 = gr.Textbox(
                        label="Initial prompt 1",
                        value="",
                        show_label=False,
                        container=False,
                        placeholder="a toy train",
                    )
                    initial_prompt2 = gr.Textbox(
                        label="Initial prompt 2",
                        value="",
                        show_label=False,
                        container=False,
                        placeholder="a toy airplane",
                    )
                    initial_prompt3 = gr.Textbox(
                        label="Initial prompt 3",
                        value="",
                        show_label=False,
                        container=False,
                        placeholder="a toy bicycle",
                    )
                    initial_prompt4 = gr.Textbox(
                        label="Initial prompt 4",
                        value="",
                        show_label=False,
                        container=False,
                        placeholder="a toy car",
                    )
                    initial_prompt5 = gr.Textbox(
                        label="Initial prompt 5",
                        value="",
                        show_label=False,
                        container=False,
                        placeholder="a toy boat",
                    )
            with gr.Row():
                # Textbox for the style prompt
                style_prompt = gr.Textbox(
                    label="Enter a style prompt",
                    placeholder="macro photo, 3d game asset",
                    scale=3,
                )
                seed = gr.Number(
                    value=1234,
            ...
            btn = gr.Button(
                "Generate a set of Style-aligned SDXL images",
            )
    # Display the generated images
    output = gr.Gallery(
        label="Style aligned text-to-image on SDXL ",
        elem_id="gallery",
        columns=5,
        rows=1,
        object_fit="contain",
