...
InversionCallback = Callable[
    [StableDiffusionPipeline, int, T, dict[str, T]], dict[str, T]
]
...
def _encode_text_sd(
    model: StableDiffusionPipeline, prompt: str
) -> tuple[dict[str, T], T]:
...
def _encode_text_sd_with_negative(
    model: StableDiffusionPipeline, prompt: str
) -> tuple[dict[str, T], T]:
    prompt_embeds = _encode_text_sd(model, prompt)
    prompt_embeds_uncond = _encode_text_sd(model, "")
...
def _next_step(
    model: StableDiffusionPipeline, model_output: T, timestep: int, sample: T
) -> T:
...
def _ddim_loop(model: StableDiffusionPipeline, z0, prompt, guidance_scale) -> T:
    all_latent = [z0]
    text_embedding = _encode_text_sd_with_negative(model, prompt)
    ...
    for i in tqdm(range(model.scheduler.num_inference_steps)):
        ...
        noise_pred = _get_noise_pred(
            model, latent, t, text_embedding, guidance_scale
        )
...
def make_inversion_callback(zts, offset: int = 0) -> List[T, InversionCallback]:

    def callback_on_step_end(
        pipeline: StableDiffusionPipeline, i: int, t: T, callback_kwargs: dict[str, T]
    ) -> dict[str, T]:
        latents = callback_kwargs["latents"]
        latents[0] = zts[max(offset + 1, i + 1)].to(latents.device, torch.float32)
